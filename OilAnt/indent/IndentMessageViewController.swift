...
class IndentMessageViewController: UIViewController, MAMapViewDelegate, AMapSearchDelegate {
...
//        self.infoLabel.text = "共\(UInt(self.totalRouteNums))条路线，当前显示第\(UInt(self.currentRouteIndex) + 1)条"  //提示信息
        ...
        self.naviRoute = MANaviRoute(path: self.route.paths[self.currentRouteIndex], withNaviType: type, showTraffic: true, start: startPoint, end: endPoint)
        
//      showTraffic: self.showTrafficSwitch.isOn
